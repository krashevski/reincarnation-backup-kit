...
- Никогда делайте push origin main, если включена защита ветки.
- Перед слиянием убедитесь, что commit для всех локальных изменений.
- Используйте feature-ветки для новых задач и экспериментов.
- Создавайте резервные ветки перед критическими изменениями.

## 📝 Обновление README через rebase в ветке feature/update-readme

### 1. Перейдите в ветку документации

```bash
git checkout feature/update-readme

### 2. Подтяните последние изменения с GitHub
```bash
git fetch origin
...
### 3. Правило проверки изменений в README:
```bash
git status
git diff
``` 
...
### 6. Создайте коммит:
```bash
git commit -m "docs: исправления и дополнения README_GIT_REBASE"
```
...
> * Вы можете спокойно проверить в вашей рабочей ветке, что всё корректно работает (i18n, документация, скрипты).
> * Можно поправить ошибки, конфликты и протестировать локально, прежде слияния с main.
> * После тестирования и проверки переход в main будет безопасным, и история останется чистой.

1. Перейдите в ветку main и обновите к последним изменениям:
```bash
git checkout main
git pull origin main
```

2. Сделайте слияние изменений из feature-ветки:
```bash
git merge feature/update-readme
```

3. Запушьте обновлённый main на GitHub:
```bash
