# REBK Lib — Мини-README

[🇬🇧 English](../EN/README_REBK_LIB_EN.md| [🇷🇺 Русский]()

## 📂 Структура папки lib

lib/
├─ logging.sh       # логирование + i18n + цвета
├─ deps.sh          # проверка и установка зависимостей
└─ (доп. модули)   # utils, guards, locks и др.

## 1️⃣ Подключение в скрипте REBK

#!/usr/bin/env bash

# путь к lib
LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/lib" && pwd)"

# подключаем логирование и i18n
source "$LIB_DIR/logging.sh"

# подключаем проверку зависимостей
source "$LIB_DIR/deps.sh"

> После этого доступны все функции:
> ok/info/warn/error/die
> say(key, ...)
> check_and_install_deps(...)

## 2️⃣ Использование логирования с i18n

### 2.1 Настройка языка
# по умолчанию ru
LANG_CODE="${LANG_CODE:-ru}"
load_messages "$LANG_CODE"

### 2.2 Примеры логирования
ok deps_ok                    # [OK] Сообщение из MSG['deps_ok']
info deps_install_try          # [INFO] Попытка установки зависимостей…
warn deps_missing_list "rsync tar gzip"
error no_script "$DISTRO" "$VERSION" "$TARGET"
die 2 "fatal_error_occurred"

> Все сообщения берутся из MSG[] в i18n/messages_ru.sh или messages_en.sh.

### 3️⃣ Функция say()
- Формат: say <key> [args...]
- Подставляет аргументы в шаблон i18n через printf

Пример в messages_ru.sh:
```bas
MSG[no_script]='%s %s — скрипт %s не найден или не является исполняемым'
```
Вызов:
```bas
say no_script "$DISTRO" "$VERSION" "$TARGET"

```
Вывод:
```bash
Ubuntu 22.04 — скрипт backup-system.sh не найден или не является исполняемым
```

## 4️⃣ Проверка зависимостей

```bash
# Проверяет команды и пытается установить отсутствующие
check_and_install_deps rsync tar gzip pv
```

- Проверка команд через command -v
- Поддерживаются пакетные менеджеры: apt-get, dnf, yum, pacman, zypper
- Логируются отсутствующие пакеты через warn
- При успешной проверке выводится [OK]

## 5️⃣ Цвета и потоки

| Функция | Цвет| Поток | Код возврата |
| ------------------------------------ |
| ok | зелёный	| stdout | 0 |
| info |синий	| stdout | 0 |
| warn | жёлтый |	stderr | 0 |
| error | красный | stderr | 1 |
| die | красный | stderr | завершение скрипта с кодом 1 или указанным |

- Цвета автоматически отключаются, если stdout не tty или FORCE_COLOR=0
- Логи сохраняются в RUN_LOG, если задана переменная

## 6️⃣ Пример полного скрипта

```bash
#!/usr/bin/env bash

LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/lib" && pwd)"
source "$LIB_DIR/logging.sh"
source "$LIB_DIR/deps.sh"

# язык
LANG_CODE=ru
load_messages "$LANG_CODE"

# проверка зависимостей
check_and_install_deps rsync tar gzip pv

# логирование
ok "Скрипт запущен успешно"
info "Начинаем резервное копирование..."
warn "Обнаружены старые версии пакетов"
error "Не удалось найти скрипт"
die 1 "Фатальная ошибка"
```