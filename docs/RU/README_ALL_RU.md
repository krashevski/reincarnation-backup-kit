...
- `backup-restore-firefox.sh` - Создание резервных копий и восстановление профиля и пользовательских данных Firefox.

### Библиотеки
lib/ — Стандартная библиотека REBK.
Содержит только функции и не имеет побочных эффектов.

maintenance/ — Исполняемые утилиты для обслуживания.
Они включают библиотеки и управляют жизненным циклом.

i18n/ — Ресурсы интернационализации.
Содержит языковые файлы с локализованными сообщениями для пользователя.
Используется библиотекой i18n для обеспечения согласованного, переводимого вывода.
